...
class UserRSerializer(serializers.HyperlinkedModelSerializer):
    ...
    # Character complexity check
        if not re.search(r'[A-Z]', value) or not re.search(r'[a-z]', value) or not re.search(r'\d',
...
class UPRS(serializers.Serializer):
    ...
    def validate(self, attrs):
        try:
            ...
            user.save()
            return super().validate(attrs)
        except DjangoUnicodeDecodeError:  # except DjangoUnicodeDecodeError as identifier:
